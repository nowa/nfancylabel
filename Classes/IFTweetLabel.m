...
- (void)handleButton:(id)sender
{
	...
	//NSLog(@"keys: %@", self.linkHref);
	//NSLog(@"tag: %d", [sender tag]);
	//NSLog(@"href: %@", [self.linkHref objectAtIndex:[sender tag]]);
	//NSLog(@"text: %@", [buttonTitle substringFromIndex:1]);
	//NSLog(@"IFTweetLabel: handleButton: sender = %@, title = %@", sender, buttonTitle);

	NSString *text = self.sourceText;
	...
	for (NSString *expression in expressions)
	{
		...
		{
			//NSLog(@"button: %@, match: %@, substring: %@", buttonTitle, match, [buttonTitle substringFromIndex:1]);
			if ([match hasPrefix:buttonTitle] || [match isMatchedByRegex:[NSString stringWithFormat:@"<a\\s+href=\"%@\"[^>]*>%@<\\/a>", [self.linkHref objectAtIndex:[sender tag]], [buttonTitle substringFromIndex:1]]])
			{
				NSString *target = [expression isEqualToString:@"<a\\s+href=\"([^\"]+)\"[^>]*>([^>]*?)<\\/a>"] ? [self.linkMap objectForKey:match] : match;
				[[NSNotificationCenter defaultCenter] postNotificationName:IFTweetLabelURLNotification object:target];
			}
		}
	}
}
...
- (NSString *)scanLinks:(NSString *)text
{
	NSString *match;
	NSString *expression = @"<a\\s+href=\"([^\"]+)\"[^>]*>([^>]*?)<\\/a>";
	NSEnumerator *enumerator = [text matchEnumeratorWithRegex:expression];
	
	//if ([text isMatchedByRegex:expression]) {
	while (match = [enumerator nextObject]) {
		NSString *hrefString, *titleString;
		hrefString = [match stringByMatching:expression capture:1];
		titleString = [match stringByMatching:expression capture:2];
		[self.linkMap setValue:hrefString forKey:match];
		[self.linkHref addObject:hrefString];
	}
	
	NSLog(@"%@", self.linkMap);
	
	return [text stringByReplacingOccurrencesOfRegex:expression withString:@"·($2)"];
}
...
- (void)createButtonsWithText:(NSString *)text atPoint:(CGPoint)point
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject]) {
			...
			[self createButtonWithText:[match stringByReplacingOccurrencesOfRegex:@"·\\(([^\\)]*)\\)" withString:@"#$1"] withFrame:matchFrame withTag:self.linkCount];
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
	{
		...
		self.linkHref = [[NSMutableArray alloc] init];
		self.sourceText = @"";
		self.linkCount = 0;
...
- (void)setText:(NSString *)text
{
	...
	self.linkCount = 0;
	[self.linkMap removeAllObjects];
	[self.linkHref removeAllObjects];
	[self.label setText:[self scanLinks:text]];
